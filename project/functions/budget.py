...
def createBudget(budgetreq:BudgetReq):
    #Get the user's budget record document
    userbudget = col.find_one({"_id": budgetreq.pb_identifier})

    #Calculate the values needed for new budget
    uuidcode = str(uuid.uuid1())
    now = datetime.now()
    year = now.strftime("%Y")
    mon = now.strftime("%m")
    calmon = now.strftime("%b")
    monlastday = str(calendar.monthrange(int(year), int(mon))[1])
    ...
    if budgetreq.budget_type == "Annual":
        ...
        bperiod = "01 " + calmon + " " + year + " - " + monlastday + " " + calmon + " " + year
        start = datetime.strptime(year+"/"+mon+"/1",'%Y/%m/%d')
        end = datetime.strptime(year+"/"+mon+"/"+monlastday,'%Y/%m/%d')
    ...
    #Append new budget to budget's document
    userbudget["budgets"][uuidcode] = bitem

    #Update new budget document to mongodb
